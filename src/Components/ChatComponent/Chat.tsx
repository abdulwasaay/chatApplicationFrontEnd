...
const ChatComp: React.FC<ChatCompInterface> = ({ setHide }) => {
    ...
    return (
        ...
        //   <ChatInput />
        // </div>
        <div className="flex flex-col h-screen  w-full bg-[#232775] relative pb-[50px]" >
            <div className="flex items-center justify-between px-3 sm:px-1 md:px-4 py-2 text-white shadow bg-[#3f4396] ">
                <div>
                    <button onClick={backNavigate}><NavigateBeforeIcon /></button>
                    <Button sx={{ textTransform: "none" }} onClick={toggleDrawer("right", true)}><div className="flex items-center gap-2 sm:gap-2 md:gap-4 cursor-pointer text-[white]">
                        <div className="w-[30px] h-[30px] pl-2 pr-2 pt-1 pb-2 sm:w-[25px] sm:h-[25px] md:w-[34px] md:h-[34px] sm:pl-2 sm:pr-2 sm:pt-[1.5px] sm:pb-1 md:pl-3 md:pr-3 md:pt-[5px] md:pb-[5px] rounded-full" style={{ background: profileColors[userFirstLetter.toUpperCase() && userFirstLetter.toUpperCase()] }}>
                            {userFirstLetter.toUpperCase()}
                        </div>
                        <p className="font-bold text-[16px] sm:text-[13px] md:text-[20px]">{user?.name}</p>
                    </div></Button>
            ...
            <div className="flex-1 overflow-y-auto p-4">
                {/* Sample Messages */}
                <ChatMessage isOwnMessage={false} message="Hello! How are youdsa dsadasdasdas adfsafadsdsadsa asdsa adda as eqre v qerwqwe reerw erwer erwrwe?" />
                <ChatMessage isOwnMessage={true} message="I'm good, thanks!" />
                <ChatMessage isOwnMessage={false} message="What about you?" />
                <ChatMessage isOwnMessage={true} message="I'm good, thanks!" />
                <ChatMessage isOwnMessage={false} message="What about you?" />
